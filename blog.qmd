---
title: "Data Aquisition Blog Post"
author: "Bekah Jensen"
format: html
execute:
    echo: false
...
The table above shows the first few row of the data, this gives us an idea of what variable we are woking with. For the purpose of this analysis we will be looking at what variables affect graduation rates. 
```{python}
data.describe()
```
...
The following table is the count of N/A values after taking the average graduation rate, in state tuition, and out of stat tuition for each state and filling the na values with those mean values. Note there are still some unknown values for the categorical values.
...
The following table is the count of occurances for each state. Note that there is not data on all of the states in the U.S. this is due to the amount of data pulled from the API, only 1,000 schools were accounted for in the data, therefore the data will be skewed. 
...
It is interesting to note that the District of Columbia, which is a territory has the highest graduation rate, which Connecticut following as a close second. Again we really can't pull reliable inferences from this since the sample sizes are skewed, especially since New York, which has some of the most prestegious universities in the country, has the lowest graduation rate and only one singular occurrance in the dataset. 
...
The plot above shows in state tuition prices plotted against graduation rates, with the colors coressponding to north and south regions of the U.S. It is interesting to note that there appears to be a linear relationship between higher tuition prices and graduation rate.
...
These highlights of the data help us understand that while more information and data is needed we can see that higher tuition prices could result in larger graduation rates in U.S. colleges.
